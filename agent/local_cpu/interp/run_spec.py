...
for case in range(int(sys.argv[1]) if len(sys.argv) > 1 else 60):
    ...
    tokens = spec.propose(history, position, TOKENS, D, top, chains, phases)
    greedy = torch.tensor([[rng.randrange(vocab) for _ in range(TOKENS)] for _ in range(batch)])
    want = []
    for b in range(batch):
        out, chain, ph = emu.emu_propose(history[b].tolist(), int(position[b]), top.tolist(), SIZE, TOKENS, D, 8, TOP)
    ...
    spec.settle(tokens, greedy, p2, limit, h2, result, move_from, move_to, chains)
    for b in range(batch):
