...
def _qk_rope_cache(
    ...
    cosine = tl.load(cos + token * DIM + col, valid, other=0).to(tl.float32)
    sine = tl.load(sin + token * DIM + col, valid, other=0).to(tl.float32)
...
def qk_rope_cache(packed, q_norm, k_norm, cos, sin, position, keys, values, q_heads, prefill=False):
